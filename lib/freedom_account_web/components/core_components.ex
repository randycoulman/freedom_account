defmodule FreedomAccountWeb.CoreComponents do
  @moduledoc """
  ...
  """
  attr :rest, :global, include: ~w(disabled download href method name navigate patch type value)
  attr :class, :any
    ...
    <table class="table table-zebra">
      ...
      <tbody id={@id} phx-update={is_struct(@rows, Phoenix.LiveView.LiveStream) && "stream"}>
        ...
        <tr :for={row <- @rows} id={@row_id && @row_id.(row)}>
          ...
          <td :if={@action != []} class="font-semibold text-primary/50 w-0">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(FreedomAccountWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(FreedomAccountWeb.Gettext, "errors", msg, opts)
    end
  end
